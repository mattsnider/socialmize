...
YUI($YO).add('matt_form', function(Y) {
	// constants
var CLS_DISABLED = 'disabled',
	...
	// local
	Form, FormField, FormFieldSerializers;
	...
	FormField = {
		 ...
		 * @param  sOverrideColor {String} Optional. The color to set default text to.
		 * @static
		 */
		attachFocusAndBlur: function(elFld, sCopy, sOverrideColor) {
			var elForm = elFld.ancestor('form'),
                sNewColor = sOverrideColor || '#999',
				sOldColor = elFld.getStyle('color') || '#000',
				sValue;
			...
			elFld.on('blur', function() {
				if (Lang.trim(elFld.get('value'))) {fnUpdate(sCopy, sNewColor);}
			});
			...
			if (sCopy == sValue || '' === sValue) {fnUpdate(sCopy, sNewColor);}
		},
