...
/**
	...
	* @param int $image_max_x
	* @param int $image_max_y
	* @desc Scale rectangular image width to @link image_max_x OR height to @link image_max_y, relative to the smaller proporation, before cropping (useful for thumbnails)
	*/
	function resizeImageForCrop($image_src, $image_max_x, $image_max_y) {
		$image_src_size = getimagesize($image_src);
		
		if ($image_src_size[0]) {
			$image_src_x = $image_src_size[0];
			$image_src_y = $image_src_size[1];
			$image_dst_x = $image_src_x;
			$image_dst_y = $image_src_y;
			
			if ($image_src_x < $image_src_y) {
				$image_dst_y = round($image_src_y * $image_max_x / $image_src_x);
				$image_dst_x = $image_max_x;
			} else {
				$image_dst_x = round($image_src_x * $image_max_y / $image_src_y);
				$image_dst_y = $image_max_y;
			}
			
			$image_dst = imagecreatetruecolor($image_dst_x, $image_dst_y);
			$res = imagecopyresampled($image_dst, $this->_imgOrig, 0, 0, 0, 0, $image_dst_x, $image_dst_y, $image_src_x, $image_src_y);
			$this->_imgOrig = $image_dst;
	...
	*/
	function _cropSize($ox, $oy, $nx, $ny, $position, $function)
	{
		...
		/*
		BY COMMENTING THIS OUT, WE ENSURE SMALLER IMAGES ARE FILLED TO FIT THE DIMENSIONS WE WANT
		if (($nx > @ImageSX($this->_imgOrig)) && ($ny > @ImageSY($this->_imgOrig)))
		{
			...
			return false;
		} else if ($ny > @ImageSY($this->_imgOrig) && $nx <= @ImageSX($this->_imgOrig)) {
			$nx = ImageSX($this->_imgOrig);
		} else if ($nx > @ImageSX($this->_imgOrig) && $ny <= @ImageSY($this->_imgOrig)) {
			$ny = ImageSY($this->_imgOrig);
		}*/
		if ($ox == -1 || $oy == -1)
		{
